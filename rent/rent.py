...
async def deactivate_car(all_cars: dict):
    if show_all_cars(all_cars):
        ...
        if execute_query(query=query, params=params):
            ...
            logger.warning(msg="No such car found")
            return False
    return False
...
async def create_an_order():
    ...
    if not user:
        logger.warning(msg="No such user found.")
        return False
    ...
    if not user['is_active']:
        ...
        if not car['is_active'] and car['deactivate']:
            ...
            logger.warning(msg="This car is busy.")
            return False

    else:
        logger.warning(msg="No such user exists.")
        return False
...
async def show_all_active_orders():
    ...
    for order in all_orders:
        print(
            f"|Id: {order['id']} | User id: {order['user_id']} | Car id: {order['car_id']} | Rent data: {order['rent_at']} |")
    return True
...
async def show_all_orders():
    ...
    if not all_orders:
        ...
        logger.warning(msg="No active orders.")
        return False
...
async def finishing_order(current_date: datetime.date = None):
    ...
    if not order:
        logger.warning(msg="No such order.")
        return False
